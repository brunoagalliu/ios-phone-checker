...
export async function getConnection() {
  if (!pool) {
    ...
    pool = mysql.createPool({
      ...
      // acquireTimeout: 15000, // ❌ REMOVE THIS - not valid in mysql2
      // timeout: 60000, // ❌ REMOVE THIS - use connectTimeout instead
    });
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error.code === 'ETIMEDOUT' || error.code === 'ECONNRESET' || error.code === 'PROTOCOL_CONNECTION_LOST') {
        console.warn(`⚠️ Database timeout on attempt ${attempt}/${maxRetries} for ${data.phone_number}`);
