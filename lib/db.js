...
export async function updateFileStatus(fileId, status, additionalData = {}) {
  ...
  if (additionalData.sv_send_count !== undefined) {
    updates.push('sv_send_count = ?');
    values.push(additionalData.sv_send_count);
  }
  
  if (additionalData.sv_unsubscribe_count !== undefined) {
    updates.push('sv_unsubscribe_count = ?');
    values.push(additionalData.sv_unsubscribe_count);
  }
  
  if (additionalData.sv_blacklist_count !== undefined) {
    updates.push('sv_blacklist_count = ?');
    values.push(additionalData.sv_blacklist_count);
  }
