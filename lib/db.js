...
export async function saveUploadedFile(fileData) {
  ...
  console.log('saveUploadedFile received:', JSON.stringify(fileData, null, 2));
  ...
  const undefinedKeys = Object.keys(safeData).filter(key => safeData[key] === undefined);
  if (undefinedKeys.length > 0) {
    console.error('WARNING: Found undefined values for keys:', undefinedKeys);
    throw new Error(`Undefined values found: ${undefinedKeys.join(', ')}`);
  }
  
  console.log('safeData prepared:', JSON.stringify(safeData, null, 2));
...
export async function updateFileStatus(fileId, status, additionalData = {}) {
  ...
  // Only add fields that are explicitly provided and not undefined
  const fieldMap = {
